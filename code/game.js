...
this.size = new Vecto(0.5,0.5);

//make it go back and forth like the coin

this.wobble = Math.random() * Math.PI * 2;

}

Power.prototype.type = "powerup";*/
...
*/
Ally.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  this.status = "enemyLost";
  this.finishDelay = 1;

} else if (type == "enemy2" && this.status == null){
  this.status = "enemy2Lost";
  this.finishDelay = 1;
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "enemyLost"){
      alert('You were hit by an enemy! Try again!');
    startLevel(n);}
    else if (status == "enemy2Lost"){
      alert('You were hit by an enemy! Try again!');
    startLevel(n);}
    else if (status == "lost"){
      alert('Looks like you fell down! Try again!');
    startLevel(n);}
    /*else if (status == "lost" || status == "lavaLost"){
      alert('Oh no! You got sucked into some quicksand! Try again, you have to find all the map pieces!');
    startLevel(n);}*/
    else if (n < plans.length - 1){
    alert('Lets continue the adventure!');
    startLevel(n + 1);}
  if (n == 4){
      alert("Hurray you made it to the end of the adventure. Thanks for playing!"
      startLevel(0)
    }
      else
        console.log("You win!");
  ...
  startLevel(0);
  alert("It's time for another adventure with our good friend Blox! Collect all the coins through the level to move on. Be sure to avoid the deadly black blocks."); 
}
